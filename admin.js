...
window.deleteStudent = function(id, year) {
    if(confirm(`Are you sure you want to completely delete the student account? This action cannot be undone.`)) {
        ...
        const studentToDelete = allStudents.find(s => {
            if (s._id) return String(s._id) === targetId;
            return String(s.rollNo) === targetId && (s.year || '') === (year || '');
        });
        ...
        allUsers = allUsers.filter(u => {
            if (studentToDelete._id) {
                return String(u._id) !== String(studentToDelete._id);
            }
            // For legacy students without _id, delete the matching rollNo and year
